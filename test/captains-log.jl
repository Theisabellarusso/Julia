...
@testset verbose = true "tests" begin
    ...
    @testset "1. Generate a random planet" begin
        ...
        random_planets = [random_planet() for _ = 1:repeats]
    ...
    @testset "2. Generate a random starship registry number" begin
        random_registry = [random_ship_registry_number() for _ = 1:repeats]
        ...
        @testset "all registry numbers have the correct numeric range" begin
            @test all([
                1000 ≤ parse(Int, regno[5:end]) ≤ 9999 for
                regno in random_registry
            ])
        end
    end

    @testset "3. Generate a random stardate" begin
        random_stardates = [random_stardate() for _ = 1:repeats]
    ...
    @testset "4. Generate a rounded stardate" begin
        ...
        @testset "stardates have one decimal place" begin
            @test all([
                isapprox(stardate, round(stardate, digits = 1), atol = 1e-8) for
                stardate in rounded_stardates
            ])
        end
    end

    @testset "5. Pick some random starships from a list" begin
        starships = unique([random_ship_registry_number() for _ = 1:100])
        lengths = rand(1:20, 100)
        chosen_starships =
            [pick_starships(copy(starships), len) for len in lengths]

        @testset "starships are taken from the input list" begin
            @test all([
                all([starship ∈ starships for starship in fleet]) for
                fleet in chosen_starships
            ])
        end

        @testset "all chosen starships are unique" begin
            @test all([
                length(fleet) == length(unique(fleet)) for
                fleet in chosen_starships
            ])
        end
        ...
        @testset "starships are chosen at random" begin
            @test all([
                sum(fleet .== starships[1:length(fleet)]) ≤ 4 for
                fleet in chosen_starships
            ])
        end
    end
